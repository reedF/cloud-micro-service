...
/**
 * 注： 1.SpringBootApplication默认扫描当前包及子包，如需扫描其他包请手动加入
 * 2.Hystrix是在“代理”模式下运行的，@HystrixCommand在本类内部未经过aop加强时是无效的，
 * 顾对@HystrixCommand这类命令的使用，需通过另一个类@Autowired注入此类时，方可生效使用，
 * 参见：http://stackoverflow
 * .com/questions/29523604/hystrix-javanica-fallback-not-working
 * -in-spring-cloud-1-0 3.@HystrixCommand的fallbackmethod必须是本类内部定义，继承或其他类中的无效
 * 
 * @author reed
 *
 */
